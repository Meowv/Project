...
//增加百分比
function addPercent() {
    var $java = $('.java').find('.percent').text(),
        $html = $('.html').find('.percent').text(),
        $css = $('.css3').find('.percent').text(),
        $pg = $('.phonegap').find('.percent').text(),
        $jq = $('.jquery').find('.percent').text(),
        $bt = $('.bootstrap').find('.percent').text(),
        $net = $('.net').find('.percent').text(),
        $sql = $('.sql').find('.percent').text();

    if ($html == html) {
        clearInterval(timer);
    } else {
        if ($net == net) {
            $('.net').find('.percent').text(net);
        } else {
            $('.net').find('.percent').text(i);
        }
        if ($sql == sql) {
            $('.sql').find('.percent').text(sql);
        } else {
            $('.sql').find('.percent').text(i);
        }
        if ($jq == jq) {
        ...
        if ($pg == pg) {
            $('.phonegap').find('.percent').text(pg);
        } else {
            $('.phonegap').find('.percent').text(Math.floor((pg - $pg) / k) + i);
        }
        if ($css == css) {
        ...
        if ($java == java) {
            $('.java').find('.percent').text(java);
        } else {
            $('.java').find('.percent').text(Math.floor((java - $java) / k) + i);
        }
...
function section4() {
    ...
    move('.num8 .simboo').scale(1).end(function() {
        $('.num8 .progress-content').animate({
            'opacity': 1
        }, 700).addClass('move-left');
    });
    move('.num9 .simboo').scale(1).end(function() {
        $('.num9 .progress-content').animate({
            'opacity': 1
        }, 700).addClass('move-right');
    });
...
function section5(){
    ...
    move('.my-venus .content-box').set('transform','rotateY(0deg)').set('opacity',1).duration('1.9s').end();
    move('.my-company-pro .content-box').set('transform','rotateY(0deg)').set('opacity',1).duration('2.1s').end();
}
//section6动画
function section6() {
    ...
    $(document).ready(function(){
        ...
        move('.num8 .progress-content').set('opacity',0).end();
        move('.num9 .progress-content').set('opacity',0).end();
        ...
        move('.num8 .simboo').scale(0).end();
        move('.num9 .simboo').scale(0).end();
        ...
        move('.my-venus .content-box').set('transform','rotateY(270deg)').set('opacity',0).end();
        move('.my-company-pro .content-box').set('transform','rotateY(270deg)').set('opacity',0).end();
        ...
        $('#fullpage').fullpage({
            ...
            navigationTooltijava: ['欢迎界面', '关于我', '专业技能', '实习经历', '个人作品', '联系方式'],
...
console.log('听说您在寻找一匹千里马？');
console.log("这是他的邮箱:%cHackxing@foxmail.com","color:red");
console.log("要不打个电话试试:%c 134-7799-6338","color:red");
});//readyfunction结束